...
_isgun = false;
_details = [_item] call sd_fnc_fetchCfgDetails;
...
if(_bool) then
{
	switch((_details select 6)) do
	{
		...
		case "CfgVehicles":
		{
			...
			{ 
				{[_x,true,true,false,true] spawn sd_fnc_handleItem; } foreach _items;
			};
		};
		case "CfgMagazines":
		{
			...
			if(_isgun) then
			{
				...
				switch(_details select 5) do
				{
							...
							else
						{
								...
								else
							{
									...
									else
								{
									...
									if(!isNil {_items}) then
									{
										{[_x,true,false,false,true] spawn sd_fnc_handleItem;} foreach _items;
									};
								};
							};
						};
					...
					case 201:
					{
						...
						{
							private["_type"];
							_type = [_item,201] call sd_fnc_accType;
							if(_ongun) then
							{
								switch (_type) do
								{
								...
								else
							{
									...
									else
								{
									private["_wepItems","_action","_slotTaken"];
										...
										if(_slotTaken) then {
											_action = [localize "STR_Script_AttachmentMSG",localize "STR_Script_Attachment",localize "STR_Script_Weapon",localize "STR_Script_Inventory"] call BIS_fnc_guiMessage;
											if(_action) then {
												switch(_type) do {
													case 1: {player addPrimaryWeaponItem _item;};
													case 2: {player addSecondaryWeaponItem _item;};
					...
					case 301:
					{
						...
						{
							private["_type"];
							_type = [_item,301] call sd_fnc_accType;
					...
					case 101:
					{
						...
						{
							private["_type"];
							_type = [_item,101] call sd_fnc_accType;
