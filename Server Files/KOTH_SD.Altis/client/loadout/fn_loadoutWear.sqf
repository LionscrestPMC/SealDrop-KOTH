...
if(_goggles != "") then {
    _handle = [_goggles,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_headgear != "") then {
    _handle = [_headgear,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_uniform != "") then {
    _handle = [_uniform,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_vest != "") then {
    _handle = [_vest,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_backpack != "") then {
    _handle = [_backpack,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_prim != "") then {
    _handle = [_prim,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_seco != "") then {
    _handle = [_seco,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
};
if(_launcher != "") then {
    _handle = [_launcher,true,false,false,false] spawn sd_fnc_handleItem;
    waitUntil {scriptDone _handle};
};
{
    _handle = [_x,true,false,false,false] spawn sd_fnc_handleItem; 
    waitUntil {scriptDone _handle};
} foreach _items;
